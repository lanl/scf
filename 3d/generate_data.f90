...
!
! Functionality:
!   The script is to generate training dataset for training the 3D version of
...
program test
    ...
    noise = random_pdf(nm, pdf=[0.1, 0.5, 1.0, 0.5], range=[0.0, 0.5], seed=seeds(6))
    ...
    do i = blkrange(rankid, 1), blkrange(rankid, 2)
        ...
        if (nmeq(i) >= 1) then
            ...
            do while (l1 < nmeq(i) .and. l < 50*nmeq(i))
                ...
                dist = irand(range=[0, 2], seed=seeds(40) + i + l)

                pick1 = irand(range=[dist + 1, p%n1 - dist], seed=seeds(42) + i + l)
                pick2 = irand(range=[dist + 1, p%n2 - dist], seed=seeds(43) + i + l)
                pick3 = irand(range=[dist + 1, p%n3 - dist], seed=seeds(44) + i + l)

                if (any(pf(pick1 - dist:pick1 + dist, pick2 - dist:pick2 + dist, pick3 - dist:pick3 + dist) > 0)) then
                    dist = 3
