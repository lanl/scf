...
def pad_like(source, target):
    ...
    return F.pad(source, [pX // 2, pX - pX // 2, pY // 2, pY - pY // 2, pZ // 2, pZ - pZ // 2])
...
def upsample(l, scale_factor=2, mode='trilinear'):
    ...
    if mode == 'trilinear':
        return F.upsample(l, scale_factor=scale_factor, mode='trilinear', align_corners=False)
    else:
...
class SpatialAttention(nn.Module):
    def __init__(self, in_channels, kernel_size=7):
        ...
        self.convout = nn.Conv3d(3, 1, kernel_size=kernel_size, padding=padding, bias=False)
...
class SpatialChannelAttention(nn.Module):
    ...
    def __init__(self, in_channels_encoder, out_channels, channel_ratio=16, spatial_kernel=7):
        ...
        self.cconv = nn.Conv3d(self.n, out_channels, kernel_size=1, bias=False)
    ...
    def forward(self, e1, e2):
        ...
        c, d, h, w = s[1:]
        c = c*2
        sag = sag.repeat(1, c, 1, 1, 1)
        cag = cag.repeat(1, 1, d, h, w)
        out = torch.cat((e1, e2), dim=1) * sag * cag

        return out


## Decoder -- with multi-modal fusion
class mtl_decoder_fusion(nn.Module):
    ...
    def __init__(self, l1, l2, l3, out_ch=1, bn=False, out_activation=None, last_kernel_size=1):
        ...
        if self.bn == 'batch':
            layers.append(nn.BatchNorm3d(self.out_ch))
