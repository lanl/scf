...
program test
    ...
    nl = [nint(rescale(fs(1:nm/2), range=[30.0, 55.0])), nint(rescale(fs(nm/2 + 1:nm), range=[30.0, 55.0])*0.4)] !*1.25
    pm = random_order(nm)
    fs = fs(pm)
    nl = nl(pm)
    ...
    nsmooth3 = random(nm, range=[0.0, 1.5])
    where (nsmooth1 < 0.5) nsmooth1 = 0.0
    where (nsmooth2 < 0.5) nsmooth2 = 0.0
    where (nsmooth3 < 0.5) nsmooth3 = 0.0
    ...
    nmeq = nint(rescale(nf*1.0, [200.0, 5000.0]))
