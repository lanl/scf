...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        data['img'] = torch.cat((pp, ps, sp, ss), dim=0)
...
class mtlnet(pl.LightningModule):
    def __init__(self, lr: float = 1.0e-4):
        ...
        self.lr = lr
        self.in_channels_img = 4
        self.in_channels_meq = 1
        ...
        self.encoder1_img = resu1(self.in_channels_img, self.l1)
        self.encoder2_img = resu2(self.l1, self.l2)        
        self.encoder1_meq = resu1(self.in_channels_meq, self.l1)
    ...
    def forward(self, x):
        ...
        out_encoder3 = self.encoder3(maxpool(torch.cat((out_encoder2_img, out_encoder2_meq), dim=1), 2))
...
if __name__ == '__main__':
    ...
    if opts.input is None and opts.check is not None:
        ...
        with tqdm(total=len(v), desc='', unit='image') as pbar:
            ...
            for (input, target) in valid_loader:
                ...
                for i in range(0, input['img'].shape[0]):
                    ...
                    ax[0][0].imshow(im[0], cmap='gray'), ax[0][0].set_title('PP')
                    ax[0][1].imshow(im[1], cmap='gray'), ax[0][1].set_title('PS')
                    ax[0][2].imshow(im[2], cmap='gray'), ax[0][2].set_title('SP')
                    ax[1][0].imshow(im[3], cmap='gray'), ax[0][0].set_title('SS')
                    ax[1][1].imshow(fsem, cmap='viridis'), ax[0][1].set_title('fsem')
                    ax[1][2].imshow(fdip, cmap='jet'), ax[0][2].set_title('fdip')

                    fsem = get_numpy(predict['fsem'][i])
                    fdip = get_numpy(predict['fdip'][i])

                    ax[2][0].imshow(mq, cmap='jet'), ax[1][0].set_title('meq')
                    ax[2][1].imshow(fsem, cmap='viridis'), ax[1][1].set_title('fsem')
                    ax[2][2].imshow(fdip, cmap='jet'), ax[1][2].set_title('fdip')

                    plt.show()

                pbar.update(input['img'].shape[0])
                l = l + 1
    ...
    if opts.input is not None:
        ...
        img['img'] = torch.cat((pp, ps, sp, ss), dim=1)
        img['meq'] = read_array(opts.input + '.meq', (1, 1, n1, n2))
