...
def upsample(l, scale_factor=2, mode='bilinear'):
    ...
    if mode == 'bilinear':
        return F.upsample(l, scale_factor=scale_factor, mode='bilinear', align_corners=True)
    else:
...
class mtl_decoder(nn.Module):
    ...
    def forward(self, x, out_encoder1_a, out_encoder2_a, out_encoder1_b, out_encoder2_b, l):
        ...
        up3 = self.decoder2(torch.cat((up3, out_encoder2_a, out_encoder2_b), dim=1))

        up2 = upsample_like(up3, out_encoder1_a)
        up2 = self.decoder1(torch.cat((up2, out_encoder1_b, out_encoder1_b), dim=1))
